...
# Market scans are keyed by the US-Eastern calendar date so the nightly warm job
# (which runs at 22:30 ET, right after the 21:00 ET sync) and the route (which may
# be hit at any time, from any server timezone) always agree on the same cache key.
...
def effective_avg_volume(
    ...
    """Prefer the fundamentals avg_volume; fall back to the computed 20-day OHLCV average."""
...
#   4. `agg` -- collapses that into one row per ticker: last close/open/volume
#      (rn=1), prior close (rn=2), a 20-day avg volume, and the 252-day high/low.
...
_SCAN_SQL = text(
    ...
    agg AS (
        SELECT
            ...
            AVG(CASE WHEN rn <= 20 THEN volume END) AS avg_volume_20d,
...
def _fetch_scan_data(
    ...
    result = db.execute(_SCAN_SQL, {"cap_date": cap_date, "lookback_days": lookback_days})
    rows = [dict(row) for row in result.mappings().all()]

    if not rows:
        # Either there's no data at all, or every ticker failed the
        # `agg.last_close IS NOT NULL` join filter -- fall back to a cheap direct
        # lookup so the payload can still report an accurate as_of_date.
...
def market_scans_cache_key(as_of: Optional[date_type] = None) -> str:
    """Redis cache key for a day's market scans payload (Redis key: market:scans:<date>)."""
    day = as_of or datetime_type.now(_EASTERN).date()
    return f"market:scans:{day.isoformat()}"
