...
import { ArrowLeft, ChevronDown, Info, Loader2, RefreshCcw, ScrollText, X } from 'lucide-react';
...
const normalizeBasketTickers = (basketInput: string) => {
  const ordered: string[] = [];
  const seen = new Set<string>();
  for (const raw of basketInput.split(',')) {
...
function Drawer({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm">
      ...
      <div className="deco-panel deco-scroll absolute right-0 top-0 h-full w-full max-w-2xl overflow-y-auto p-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="deco-kicker">Panel</div>
...
export default function BacktesterWorkspace({
  ...
  const fallbackTicker = initialTickers?.[0]?.toUpperCase() || 'SPY';
  const [request, setRequest] = useState<BacktestRunRequest>(() => createDefaultRequest(fallbackTicker));
  const [basketInput, setBasketInput] = useState((initialTickers?.length ? initialTickers : [fallbackTicker]).join(', '));
  const [activeDrawer, setActiveDrawer] = useState<'trade-log' | 'metric-guide' | null>(null);
  const [advancedOpen, setAdvancedOpen] = useState(false);

  useEffect(() => {
    if (initialTickers?.length) {
      const normalized = initialTickers.map((ticker) => ticker.toUpperCase());
      setBasketInput(normalized.join(', '));
      setRequest(createDefaultRequest(normalized[0]));
    }
  }, [initialTickers]);

  const resolvedTickers = useMemo(() => normalizeBasketTickers(basketInput), [basketInput]);
  const primaryTicker = resolvedTickers[0] ?? fallbackTicker;
  ...
  const { data: stock } = useQuery({
    queryKey: ['stock', primaryTicker],
    queryFn: () => stocksAPI.getStock(primaryTicker),
    enabled: resolvedTickers.length > 0,
  });
  ...
  const mutation = useMutation({
    mutationFn: (payload: BacktestRunRequest) => stocksAPI.runGlobalBacktest(payload),
  });
  ...
  const resetDefaults = () => {
    setRequest(createDefaultRequest(primaryTicker));
    setBasketInput(primaryTicker);
    setAdvancedOpen(false);
  };

  const applyPreset = (preset: ExamplePreset) => {
    const next = preset.apply(primaryTicker);
    const nextTickers = next.tickers ?? [primaryTicker];
    setRequest((prev) => ({
      ...prev,
      ...next,
      tickers: nextTickers,
  ...
  return (
    <>
      <div className="container-custom space-y-8 py-8">
        ...
        {returnHref ? (
          <Link href={returnHref} className="inline-flex items-center gap-2 text-xs text-muted-foreground transition-colors hover:text-primary">
      ...
      <form className="grid gap-6 xl:grid-cols-[1.06fr_0.94fr]" onSubmit={onSubmit}>
        <div className="space-y-6 xl:sticky xl:top-28 xl:self-start">
          ...
          <section className="deco-panel bg-card p-0">
            <button
              type="button"
            ...
            >
              <div>
                <div className="deco-kicker">Advanced</div>
                <h2 className="text-lg font-bold">Parameter Tuning</h2>
                <p className="text-sm text-muted-foreground">Collapsed by default to keep the workspace compact.</p>
              </div>
              <ChevronDown className={cn('h-5 w-5 text-primary transition-transform', advancedOpen && 'rotate-180')} />
            </button>
            {advancedOpen ? (
              <div className="border-t border-primary/14 p-6">
                <div className="flex items-center justify-between gap-4">
                ...
                <div className="mt-4 grid gap-4 sm:grid-cols-2">
                  <Field label="Objective" help="Sharpe Ratio is the safest default when you want better risk-adjusted results instead of raw return only." suggestion="Sharpe Ratio">
                    <select value={request.tuning?.objective ?? 'sharpe_ratio'} onChange={(event) => setRequest((prev) => ({ ...prev, tuning: { ...(prev.tuning ?? { enabled: false, objective: 'sharpe_ratio', max_combinations: 100, parameter_ranges: {} }), objective: event.target.value as BacktestObjective } }))} className="h-12 w-full border-0 border-b-2 border-input bg-transparent px-0 text-sm outline-none focus:border-ring">
                      {OBJECTIVES.map((objective) => <option key={objective.value} value={objective.value}>{objective.label}</option>)}
                    </select>
                  </Field>
                  <Field label="Max Combinations" help="Keep this under 100 unless you already know the parameter space is small and clean." suggestion="25 to 100">
                ...
                <div className="mt-4 space-y-3">
                  {strategy.fields.map((field) => {
                    ...
                    return (
                      <div key={field.key} className="border border-primary/14 bg-muted/10 p-4">
          ...
          <section className="deco-panel min-h-[220px] bg-card p-6">
            ...
            <div className="mt-5 h-[320px] border border-primary/16 bg-background/70 p-3">
              {mutation.isPending ? (
                ...
                <div className="flex h-full items-center justify-center text-center text-sm text-destructive">{(mutation.error as Error).message || 'Backtest failed.'}</div>
              ) : result ? (
                ...
                <div className="flex h-full items-center justify-center text-center text-sm text-muted-foreground">Run a configuration to populate the equity curve, summary metrics, and trade log.</div>
              )}
            </div>
            {result ? (
              <div className="mt-5 space-y-5">
                ...
                <div className="flex flex-wrap gap-3">
                  <Button type="button" variant="outline" onClick={() => setActiveDrawer('trade-log')}>
                    <ScrollText className="h-4 w-4" />
                    Trade Log
                  </Button>
                  <Button type="button" variant="ghost" onClick={() => setActiveDrawer('metric-guide')}>
                    <Info className="h-4 w-4" />
                    Metric Guide
                  </Button>
                </div>
                {result.buy_and_hold ? (
                  <div className="border border-secondary/30 bg-secondary/10 p-4">
                ...
                {result.tuning_summary?.enabled ? <div className="border border-primary/16 bg-muted/10 p-4 text-sm text-muted-foreground">Tuned on {result.tuning_summary.objective}. Evaluated {result.tuning_summary.evaluated_combinations} combinations.</div> : null}
              </div>
            ) : null}
          </section>
      ...
      <Drawer title="Trade Log" open={activeDrawer === 'trade-log'} onClose={() => setActiveDrawer(null)}>
        {result ? (
          <div className="overflow-x-auto">
            <table className="deco-table min-w-[1040px] text-sm">
              <thead>
                <tr>
                  <th>Ticker</th><th>Entry</th><th>Exit</th><th>Entry Px</th><th>Exit Px</th><th>Shares</th><th>Net PnL</th><th>Return</th><th>Bars</th><th>Exit Reason</th>
                </tr>
              </thead>
              <tbody>
                {result.trade_log.length > 0 ? result.trade_log.map((trade, index) => (
                  <tr key={`${trade.ticker}-${trade.entry_date}-${index}`} className="border-b border-primary/10">
                    <td className="font-semibold text-primary">{String(trade.ticker ?? '')}</td>
                    <td>{String(trade.entry_date ?? '')}</td>
                    <td>{String(trade.exit_date ?? '')}</td>
                    <td>{formatCurrency(Number(trade.entry_price ?? 0))}</td>
                    <td>{formatCurrency(Number(trade.exit_price ?? 0))}</td>
                    <td>{formatNumber(Number(trade.shares ?? 0))}</td>
                    <td>{formatCurrency(Number(trade.net_pnl ?? 0))}</td>
                    <td>{formatPercent(Number(trade.net_return_pct ?? 0), { mode: 'percent' })}</td>
                    <td>{formatNumber(Number(trade.holding_period_bars ?? 0))}</td>
        ...
        ) : <div className="text-sm text-muted-foreground">Run a study first.</div>}
      </Drawer>

      <Drawer title="Metric Guide" open={activeDrawer === 'metric-guide'} onClose={() => setActiveDrawer(null)}>
